...
private const val MAX_HP = 150.0
...
private const val ATTACK_RANGE = 2.5
private const val ATTACK_RANGE_SQ = ATTACK_RANGE * ATTACK_RANGE
private const val ATTACK_COOLDOWN = 40
private const val HITBOX_WIDTH = 7.5f
private const val HITBOX_HEIGHT = 7.5f
...
fun setupBossAI(app: ECS, plugin: JavaPlugin) {
    ...
    fun killBoss(entity: ECSEntity, spider: SpiderBody, boss: BossAI) {
        ...
        world.dropItemNaturally(loc, org.bukkit.inventory.ItemStack(org.bukkit.Material.ECHO_SHARD, 1))
        world.spawn(loc, org.bukkit.entity.ExperienceOrb::class.java) { it.experience = 100 }
    ...
    plugin.server.pluginManager.registerEvents(object : Listener {
        ...
        fun onDamage(event: EntityDamageByEntityEvent) {
            ...
            spider.location().world?.playSound(spider.location(), Sound.ENTITY_WARDEN_HURT, 1f, 1f)

            if (boss.hp <= 0) killBoss(entity, spider, boss)
        }
    }, plugin)
}
